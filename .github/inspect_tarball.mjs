 ...
 *
 *   - A file that should ship and does not. `dist/index.js` is this package's
 *     example: the manifest's `main`/`types`/`exports` all name it, every
 *     in-tree test passes without it, and the first person to notice its
 *     absence is the customer whose `import '@mudraid/adapter-node'` throws.
...
const PACKAGE_NAME = '@mudraid/adapter-node';

/** What must be in the tarball — the thing `npm install` actually delivers. */
const REQUIRED = [
  'package/package.json',
  // The manifest's `main`, `types` and `exports` all point here. Absent,
  // every consumer import fails at runtime and every consumer type-check
  // fails at build — and no in-tree test would ever have said so.
  'package/dist/index.js',
  'package/dist/index.d.ts',
  // `files` deliberately ships the TypeScript source beside the build:
  // source maps reference it, and a distributor auditing the artifact reads
  // it. If it stops shipping, that is a decision, not an accident.
...
function checkContents(distDir) {
  ...
  console.log(
    `inspect_tarball: ${tgz} carries the built entry points, the source, and ` +
